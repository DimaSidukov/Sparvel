...
DecodedData *decodeAudioFile(const char *path) {
    ...
    auto *decodedData = new DecodedData {
            .data = new float[codecContext->max_samples],
            .size = 0,
    ...
    while ((err = av_read_frame(formatContext, packet)) != AVERROR_EOF) {
        ...
        if ((err = receiveAndHandle(codecContext, frame, decodedData)) != AVERROR(EAGAIN)) {
...
//            LOGD("FRAME: %lf", decodedData->data[i * decodedData->channelCount + j]);
//        }
//    }

    // Drain the decoder.
    // drainDecoder(codecContext, frame, decodedData);
    ...
    fclose(inFile);
//
//    auto *shrinkedArr = new float[decodedData->size];
//    for (size_t i = 0; i < decodedData->size; i++) {
//        shrinkedArr[i] = decodedData->data[i];
//    }
//    free(decodedData->data);
//    decodedData->data = shrinkedArr;
    return decodedData;
    ...
    if (av_sample_fmt_is_planar(codecCtx->sample_fmt) == 1) {
        ...
        for (int s = 0; s < frame->nb_samples; ++s) {
            for (int c = 0; c < codecCtx->ch_layout.nb_channels; ++c) {
                ...
                out->data[(size + s) * codecCtx->ch_layout.nb_channels + c] = sample;
            }
        }
    } else {
        for (int s = 0; s < frame->nb_samples; ++s) {
